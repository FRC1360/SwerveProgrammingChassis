...
public class Drive extends SubsystemBase {
  ...
  // private final SwerveCamera backRightSwerveCamera;
  ...
  private SwerveDrivePoseEstimator poseEstimator =
      new SwerveDrivePoseEstimator(
          kinematics, rawGyroRotation, lastModulePositions, new Pose2d(5.0, 5.0, new Rotation2d()));
  ...
  public void updatePose() {
    ...
    // System.out.println(frontLeftSwerveCamera.getPhotonCamera().isConnected());
    ...
    for (SwerveCamera swerveCamera : swerveCameras) {
      ...
      for (var change : swerveCamera.getPipelineResults()) {
        visionEst = swerveCamera.getPhotonPoseEstimator().update(change);

        var trackedTargets = change.getTargets();

        swerveCamera.updateEstimationStdDevs(
            visionEst, trackedTargets); // change to the one we used in the method
        ...
        visionEst.ifPresent(
            est -> {
              ...
              //     > 0.5) return;
              // TODO REMOVED FOR DEBUG ADD BACK

              this.addVisionMeasurement(
                  est.estimatedPose.toPose2d(),
                  est.timestampSeconds,
                  swerveCamera.getEstimationStdDevs());
              swerveCamera.updateStructPublisher(est.estimatedPose.toPose2d());
            });
      }
    }
  }
